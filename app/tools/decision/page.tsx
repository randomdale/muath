...
export default function DecisionModelingPage() {
  ...
  const bothReady = Boolean(resultA && resultB);
  const difference = bothReady ? (resultA.expectedValue - resultB.expectedValue) : 0;
  const close = bothReady ? Math.abs(difference) < 500 : false;
